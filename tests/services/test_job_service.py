...
@pytest.fixture
def job_service(mock_database, mock_event_service, mock_job_repo):
    """Create JobService instance with mock dependencies."""
    with patch('src.services.job_service.JobRepository', return_value=mock_job_repo):
        service = JobService(mock_database, mock_event_service)
        # Replace the repo with our mock (in case patch timing differs)
...
class TestJobCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_job_with_valid_data(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_create_job_auto_generates_id(self, job_service, mock_job_repo):
        ...
        mock_job_repo.create.return_value = True
    ...
    @pytest.mark.asyncio
    async def test_create_job_sets_default_status(self, job_service, mock_job_repo):
        ...
        # Check the call arguments - repo.create is called with a dict
        call_args = mock_job_repo.create.call_args[0][0]
        assert call_args.get('status') in [None, 'pending', JobStatus.PENDING]

    @pytest.mark.asyncio
    async def test_create_job_with_missing_required_fields(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_create_job_with_business_flag(self, job_service, mock_job_repo):
...
class TestJobRetrieval:
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_all(self, job_service, mock_job_repo):
        """Test retrieving all jobs."""
        sample_jobs = [
            create_sample_job_data(job_id=str(uuid.uuid4())),
            create_sample_job_data(job_id=str(uuid.uuid4())),
        ]
        mock_job_repo.list.return_value = sample_jobs

        jobs = await job_service.get_jobs()

        assert len(jobs) == 2
        mock_job_repo.list.assert_called_once()

    @pytest.mark.asyncio
    async def test_get_jobs_with_pagination(self, job_service, mock_job_repo):
        ...
        # Should return first 5 jobs (pagination applied in service layer)
        assert len(jobs) == 5

    @pytest.mark.asyncio
    async def test_get_job_by_id(self, job_service, mock_job_repo):
        """Test retrieving a specific job by ID."""
        job_id = str(uuid.uuid4())
        job_data = create_sample_job_data(job_id=job_id)
        mock_job_repo.get.return_value = job_data
    ...
    @pytest.mark.asyncio
    async def test_get_job_by_id_not_found(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_list_jobs_by_printer(self, job_service, mock_job_repo):
        """Test filtering jobs by printer ID."""
        printer_id = 'printer_001'
        sample_jobs = [
            create_sample_job_data(job_id=str(uuid.uuid4()), printer_id=printer_id),
            create_sample_job_data(job_id=str(uuid.uuid4()), printer_id=printer_id),
        ]
        mock_job_repo.list.return_value = sample_jobs
    ...
    @pytest.mark.asyncio
    async def test_list_jobs_by_status(self, job_service, mock_job_repo):
        """Test filtering jobs by status."""
        sample_jobs = [
            create_sample_job_data(job_id=str(uuid.uuid4()), status='printing'),
            create_sample_job_data(job_id=str(uuid.uuid4()), status='printing'),
        ]
        mock_job_repo.list.return_value = sample_jobs
    ...
    @pytest.mark.asyncio
    async def test_get_business_jobs(self, job_service, mock_job_repo):
        """Test retrieving only business jobs."""
        sample_jobs = [
            create_sample_job_data(job_id=str(uuid.uuid4()), is_business=True),
            create_sample_job_data(job_id=str(uuid.uuid4()), is_business=True),
        ]
        mock_job_repo.list.return_value = sample_jobs
    ...
    @pytest.mark.asyncio
    async def test_get_private_jobs(self, job_service, mock_job_repo):
        """Test retrieving only private (non-business) jobs."""
        sample_jobs = [
            create_sample_job_data(job_id=str(uuid.uuid4()), is_business=False),
            create_sample_job_data(job_id=str(uuid.uuid4()), is_business=False),
        ]
        mock_job_repo.list.return_value = sample_jobs
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_by_date_range(self, job_service, mock_job_repo):
        """Test retrieving jobs within a date range."""
        start_date = '2024-01-01'
        end_date = '2024-01-31'
        sample_jobs = [create_sample_job_data(job_id=str(uuid.uuid4()))]
        mock_job_repo.get_by_date_range.return_value = sample_jobs

        jobs = await job_service.get_jobs_by_date_range(start_date, end_date)

        assert len(jobs) == 1
        mock_job_repo.get_by_date_range.assert_called_once_with(start_date, end_date, None)

    @pytest.mark.asyncio
    async def test_get_printer_jobs(self, job_service, mock_job_repo):
        """Test retrieving jobs for a specific printer."""
        printer_id = 'printer_001'
        sample_jobs = [create_sample_job_data(job_id=str(uuid.uuid4()), printer_id=printer_id)]
        mock_job_repo.list.return_value = sample_jobs
...
class TestJobUpdates:
    ...
    @pytest.mark.asyncio
    async def test_update_job_status(self, job_service, mock_job_repo, mock_event_service):
    ...
    @pytest.mark.asyncio
    async def test_update_job_progress(self, job_service, mock_job_repo):
        ...
        call_args = mock_job_repo.update.call_args[0]
        assert call_args[1]['progress'] == progress

    @pytest.mark.asyncio
    async def test_update_job_progress_with_material(self, job_service, mock_job_repo):
...
class TestJobDeletion:
    ...
    @pytest.mark.asyncio
    async def test_delete_job_success(self, job_service, mock_job_repo):
        """Test successful job deletion."""
        job_id = str(uuid.uuid4())
        # Job must exist and be in a non-active state
        mock_job_repo.get.return_value = {'id': job_id, 'status': 'completed'}
    ...
    @pytest.mark.asyncio
    async def test_delete_job_not_found(self, job_service, mock_job_repo):
...
class TestJobStatistics:
    ...
    @pytest.mark.asyncio
    async def test_get_job_statistics(self, job_service, mock_job_repo):
        ...
        mock_job_repo.get_statistics.return_value = expected_stats

        stats = await job_service.get_job_statistics()

        # Service adds active_jobs calculation
        assert stats['total_jobs'] == 100
        assert stats['completed_jobs'] == 80
        mock_job_repo.get_statistics.assert_called_once()
...
class TestJobValidation:
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_skips_jobs_without_id(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_handles_malformed_data(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_empty_list(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_database_error(self, job_service, mock_job_repo):
        """Test handling of database errors during job retrieval."""
        mock_job_repo.list.side_effect = Exception("Database error")
...
class TestJobCostCalculations:
    ...
    @pytest.mark.asyncio
    async def test_calculate_material_costs(self, job_service, mock_job_repo):
...
class TestActiveJobs:
    ...
    @pytest.mark.asyncio
    async def test_get_active_jobs(self, job_service, mock_job_repo):
        """Test retrieving currently active (printing) jobs."""
        active_jobs = [
            create_sample_job_data(job_id=str(uuid.uuid4()), status='running'),
            create_sample_job_data(job_id=str(uuid.uuid4()), status='running'),
        ]
        # get_active_jobs calls list() for each active status
        mock_job_repo.list.return_value = active_jobs

        jobs = await job_service.get_active_jobs()

        # Service queries multiple statuses, we return same list for simplicity
        assert len(jobs) >= 2
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_pagination_beyond_available_jobs(self, job_service, mock_job_repo):
        """Test pagination with offset beyond available jobs."""
        sample_jobs = [create_sample_job_data(job_id=str(uuid.uuid4())) for _ in range(5)]
        mock_job_repo.list.return_value = sample_jobs
    ...
    @pytest.mark.asyncio
    async def test_negative_pagination_values(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_very_large_pagination_limit(self, job_service, mock_job_repo):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_job_updates(self, job_service, mock_job_repo):
        ...
        # Both should succeed without conflict - both use update
