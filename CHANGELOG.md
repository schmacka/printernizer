...
### Changed
- Maintenance release. Version bump to force a clean Home Assistant add-on image rebuild and resolve a stale in-app version display (after updating to 2.33.0 the running app still reported 2.32.4 from a cached image layer). No functional changes.
...
### Added
- Tools: The Hornet parametric blade generator (MakerWorld)
- Tools: Gridfinity Rebuilt OpenSCAD bin generator
...
### Fixed
- Printer deletion blocked by stale jobs now shows actual error message and offers force-delete option
- API error messages were swallowed due to response format mismatch in frontend error parsing
- Dashboard printer tile showing wrong online count (incorrect query parameter and missing printing/paused statuses)
- Dashboard "Aktuelle Aufträge" showing all recent jobs instead of active/running jobs first
- External webcam URL lost after saving/editing a printer (field not propagated in API responses)
...
### Fixed
- Dashboard "Drucker online" card showing 0/0 instead of actual printer count (API response structure handling)
- Dashboard active jobs count not extracting printers array correctly
- Dashboard success rate displaying incorrect percentage (was multiplying already-percentage value by 100)

## [2.28.2] - 2026-01-17

### Added
- README with theme mockup screenshots in mockups folder
